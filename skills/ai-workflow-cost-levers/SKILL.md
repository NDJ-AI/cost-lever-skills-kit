...
**If the recommendation is to switch model tier or add an advisor, don't ship it on assumption -- hand off to the `cost-lever-eval` skill to measure it first.** Those are the only levers here with a real quality risk, so they're the ones that warrant a measured comparison rather than a paper verdict. The other levers (caching, batch, trimming, delta-recompute) are cost-only or quality-neutral when done right and don't need an eval. Match the measurement to the task shape, which `cost-lever-eval` handles: a **single-turn step** (one prompt in, one answer out -- summarize/classify/extract, i.e. most n8n/Zapier nodes) gets the light `quick_check.py` side-by-side on real inputs; only a **long-horizon agentic task** (a model looping with tools) needs the full harness. Don't send a single-turn model swap through the heavy harness -- that's the main way to overbuild this.
